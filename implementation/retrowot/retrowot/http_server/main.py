...
from thing_description_translation import enrich_affordance, align_capabilities
import uvicorn
...
class HTTPServer:
    ...
    def __init__(self):
        ...
        self.config = uvicorn.Config(self.server, host="0.0.0.0", port=8000, loop="asyncio")
        self._uvicorn_server = uvicorn.Server(config=self.config)
        self._initialize_emitters()

    def _initialize_emitters(self):
        
        logger.debug("Initializing emitters for service alignment and sub-site generation")
        ...
        if settings.thing_directory.use_thing_directory:
            ...
            self.emitters.device_affordance_emitters.on(
                "add_thing_description_to_thing_directory", add_thing_description_to_thing_directory
            )
