...
async def read_value(device, service_id, characteristic_id):
    ...
    async def read(peer: Peer):
        ...
        try:
            value = await peer.read_characteristics_by_uuid(
                uuid=CHARACTERISTIC_UUID, service=service
            )
            print(value)
            return value
    ...
    result = await _create_connection(
        device, read, str(device + service_id + characteristic_id)
    )
...
async def write_value(device, service_id, characteristic_id, value):
    ...
    await _create_connection(
        device, write, str(device + service_id + characteristic_id)
    )
...
async def notify(device, service_id, characteristic_id):
    ...
    await _create_connection(
        device, _notify, str(device + service_id + characteristic_id)
    )
    ...
    async with await open_transport_or_link(hci_device_link) as (hci_source, hci_sink):
        ...
        if DEVICE is None:
            device = Device.with_hci(
                hci_device_name, hci_device_mac, hci_source, hci_sink
            )
            DEVICE = device
